...
export class BaseAPIService {
  ...
  get<T>(url: string, params: any = {}): Observable<Response<T>> {
    ...
    const response = this.http
      .get<Response<T>>(`${this.baseApiUrl}/${url}`, options);

    return new Observable((observer: Observer<Response<T>>) => response.subscribe({
  ...
  post<T>(url: string, payload: Partial<T>): Observable<Response<T>> {
    const response = this.http
      .post<Response<T>>(`${this.baseApiUrl}/${url}`, payload);
  ...
  delete(url: string, id: string): Observable<Response<string>> {
    const response = this.http
      .delete<Response<string>>(`${this.baseApiUrl}/${url}/${id}`);

    return new Observable((observer: Observer<Response<string>>) => response.subscribe({
      next: (v) => observer.next(v),
      error: (e) => observer.error(e),
      complete: () => observer.complete(), 
    }));
